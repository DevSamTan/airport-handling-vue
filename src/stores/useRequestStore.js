...
const DEFAULT_SWAPS = [
  {
    ...
    status: "pending",
    createdAt: "2026-04-30 09:14",
  },
  {
    id: 2,
    requester: "Luca Romano",
    requesterId: 7,
    colleague: null,
    colleagueId: null,
    date: "2026-05-05",
  ...
  },
  {
    ...
    fromShift: "P",
    toShift: "M",
    reason: "Corso formazione",
    status: "pending",
    createdAt: "2026-04-30 11:05",
  },
];
...
const DEFAULT_SICK = [
  ...
  },
  {
    ...
    note: "Influenza",
    protocolNumber: "MAL-2026-0038",
    hrSentAt: "22/04/2026 14:00",
  },
  {
    id: 3,
    ...
    status: "cert_received",
    certificate: "cert.pdf",
    note: "Infortunio piazzale",
    protocolNumber: "INF-2026-0012",
    hrSentAt: "10/03/2026 09:15",
  },
];
...
export const useRequestStore = defineStore("requests", () => {
  const shiftSwaps = ref(
    loadJson("airops_swaps") ?? DEFAULT_SWAPS.map((s) => ({ ...s })),
  );
  const sickLeaves = ref(
    loadJson("airops_sick") ?? DEFAULT_SICK.map((s) => ({ ...s })),
  );
  const vacationRequests = ref(
    loadJson("airops_vacations") ?? DEFAULT_VACATIONS.map((v) => ({ ...v })),
  );

  watch(shiftSwaps, (v) => saveJson("airops_swaps", v), { deep: true });
  watch(sickLeaves, (v) => saveJson("airops_sick", v), { deep: true });
  watch(vacationRequests, (v) => saveJson("airops_vacations", v), {
    deep: true,
  });

  const pendingSwaps = computed(() =>
    shiftSwaps.value.filter((s) => s.status === "pending"),
  );
  const pendingSick = computed(() =>
    sickLeaves.value.filter((s) => s.status === "cert_pending"),
  );
  const pendingVacations = computed(() =>
    vacationRequests.value.filter((v) => v.status === "pending"),
  );
  ...
  function addVacationRequest(req) {
    vacationRequests.value.unshift({
      ...req,
      id: Date.now(),
      status: "pending",
      createdAt: new Date().toLocaleString("it-IT"),
  ...
  function approveVacation(id) {
    const v = vacationRequests.value.find((x) => x.id === id);
  ...
  function hasApprovedVacation(staffId, dateStr) {
    return vacationRequests.value.some(
      (v) =>
        v.employeeId === staffId &&
        v.status === "approved" &&
        dateStr >= v.startDate &&
        dateStr <= v.endDate,
    );
