<template>
  <div class="p-6 space-y-6">
    <!-- Tabs -->
    <div
      class="flex gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl border border-slate-200 dark:border-slate-700 w-fit flex-wrap"
    >
      <button
        v-for="tab in tabs"
        :key="tab.key"
      ...
      >
        ...
        <span
          v-if="tab.badge"
          class="ml-1.5 text-[10px] bg-red-500 text-white px-1.5 py-0.5 rounded-full"
          >{{ tab.badge }}</span
        >
      </button>
    </div>
    ...
    <div v-if="activeTab === 'swaps'" class="space-y-3">
      <div class="flex items-center justify-between">
        ...
        <select
          v-model="swapFilter"
          class="bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-200 text-xs px-3 py-1.5 rounded-lg border border-slate-300 dark:border-slate-600 focus:outline-none"
        >
      ...
      <div class="space-y-3">
        ...
        >
          <div class="flex items-start justify-between gap-4">
            <div class="flex items-center gap-3">
              <div
                class="w-9 h-9 rounded-full bg-blue-600 flex items-center justify-center text-sm font-bold text-white shrink-0"
              >
        ...
        <p
          v-if="filteredSwaps.length === 0"
          class="text-slate-400 dark:text-slate-500 text-sm text-center py-8"
        >
      ...
      <div class="space-y-3">
        ...
        >
            ...
            <div class="flex items-center gap-3">
              <div
                :class="[
                  ...
                  sick.type === 'INF' ? 'bg-rose-700' : 'bg-red-600',
                ]"
              >
              ...
              <div>
                ...
                <div class="flex items-center gap-1 mt-0.5">
                  <component
                    :is="sick.type === 'INF' ? Bandage : Thermometer"
                    :size="12"
                    :class="
                      sick.type === 'INF' ? 'text-rose-400' : 'text-red-400'
                    "
                  />
                  <p class="text-xs text-slate-500 dark:text-slate-400">
                    {{ sick.type === "INF" ? "Infortunio" : "Malattia" }}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div class="mt-3 grid grid-cols-2 gap-3 text-xs">
            ...
            <div class="bg-slate-50 dark:bg-slate-700/40 rounded-lg p-2.5">
              <p class="text-slate-500 dark:text-slate-400 mb-0.5">
                Fine presunta
              </p>
              <p class="font-medium text-slate-900 dark:text-white">
                {{ sick.endDate || "—" }}
          ...
          <div class="mt-3 flex items-center gap-2">
            ...
            <input
              ...
              @change="
                reqStore.setSickProtocol(sick.id, $event.target.value || null)
              "
          ...
          <div class="mt-3 flex flex-wrap gap-2">
            ...
            <button
              ...
              :title="
                !sick.protocolNumber
                  ? 'Inserisci il numero di protocollo prima'
                  : 'Invia comunicazione a HR'
              "
      ...
      <div class="space-y-3">
        ...
        >
          <div class="flex items-start justify-between gap-4">
            <div class="flex items-center gap-3">
              <div
                class="w-9 h-9 rounded-full bg-emerald-600 flex items-center justify-center text-sm font-bold text-white shrink-0"
              >
            ...
            <span
              :class="[
                'text-xs font-medium px-2 py-0.5 rounded-full shrink-0',
                statusClass(vac.status),
              ]"
              >{{ statusLabel(vac.status) }}</span
            >
          </div>
          ...
          <div class="mt-3 grid grid-cols-3 gap-3 text-xs">
            <div class="bg-slate-50 dark:bg-slate-700/40 rounded-lg p-2.5">
              ...
              <p class="font-medium text-slate-900 dark:text-white font-mono">
                {{ vac.startDate }}
              </p>
            </div>
            <div class="bg-slate-50 dark:bg-slate-700/40 rounded-lg p-2.5">
            ...
            <div class="bg-slate-50 dark:bg-slate-700/40 rounded-lg p-2.5">
              ...
              <p class="font-medium text-slate-900 dark:text-white">
                {{ daysBetween(vac.startDate, vac.endDate) }}gg
              </p>
            </div>
          </div>
          ...
          <div v-if="vac.status === 'pending'" class="mt-3 flex gap-2">
            <button
              @click="reqStore.rejectVacation(vac.id)"
              class="flex items-center gap-1.5 px-3 py-1.5 text-xs text-red-600 dark:text-red-400 border border-red-500/40 hover:bg-red-500/10 rounded-lg transition-colors"
            >
              <XCircle :size="13" /> Rifiuta
            </button>
            <button
              @click="reqStore.approveVacation(vac.id)"
              class="flex items-center gap-1.5 px-3 py-1.5 text-xs text-emerald-600 dark:text-emerald-400 border border-emerald-500/40 hover:bg-emerald-500/10 rounded-lg transition-colors"
            >
              <CheckCircle :size="13" /> Approva
            </button>
        ...
        <p
          ...
          class="text-slate-400 dark:text-slate-500 text-sm text-center py-8"
        >
          Nessuna richiesta ferie trovata
        </p>
      </div>
    </div>
  </div>
...
const tabs = computed(() => [
  {
    key: "swaps",
    label: "Cambi Turno",
    badge: reqStore.pendingSwaps.length || null,
  },
  {
    key: "sick",
    label: "Malattie / Infortuni",
    badge: reqStore.pendingSick.length || null,
  },
  {
    key: "vacations",
    label: "Ferie",
    badge: reqStore.pendingVacations.length || null,
  },
]);
...
const filteredVacations = computed(() =>
  vacFilter.value
    ? reqStore.vacationRequests.filter((r) => r.status === vacFilter.value)
...
function statusLabel(s) {
  return (
    { pending: "In attesa", approved: "Approvato", rejected: "Rifiutato" }[s] ||
    s
  );
}
